    ...
    var body: some Scene {
        WindowGroup("app.name.full".tr(lang)) {
            ContentView()
                ...
                .onAppear {
                    // Wire the switch-time data refresh into L10nEnvironment.didSet
                    // (view .onChange is dead under .id(resolved) rebuilds).
                    l10n.onChangeHook = { _ in store.refreshBuiltinAgentNamesForLanguage() }
                }
