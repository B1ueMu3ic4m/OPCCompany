...
public enum CLIAutoInteractionLoopStopReason: String, Codable, CaseIterable, Sendable {
    ...
    public var operatorHint: String {
        switch self {
        case .none:
            return "当前循环仍可继续，但下一轮仍必须由 OPC 生成明确输入。".L()
        case .missingTaskContext:
            return "请先绑定任务、工作项或技术负责人目标，再允许自动交互循环。".L()
        case .invalidTurnLimit:
            return "最大轮次必须在 1 到 ".L() + "\(CLIAutoInteractionLoopGate.hardTurnLimit)" + " 之间。".L()
        case .nonOPCGeneratedInput:
            return "自动循环只允许发送 OPC 生成的下一步文本，不能接收手工粘贴或外部通道原文。".L()
        case .unsafeInput:
            return "下一步文本必须是一行非空内容，不能包含换行。".L()
        case .maxTurnsReached:
            return "循环已到达预设上限，需要技术负责人查看上下文后重新发起。".L()
        case .authenticationBlocked:
            return "请先完成对应命令行工具登录，再由技术负责人重新发起。".L()
        case .busy:
            return "请等待当前命令行任务结束，避免向同一席位追加输入。".L()
        case .transientFailure:
            return "请先使用恢复建议或手动重试入口处理临时异常。".L()
        case .timedOut:
            return "本轮等待已超时，系统不会中断终端席位，也不会继续追加输入。".L()
        case .completedTurn:
            return "命令行已经报告本轮结束，循环无需继续追加输入。".L()
        }
    }
}
...
public struct CLIAutoInteractionLoopState: Codable, Hashable, Sendable {
    ...
    public var summaryText: String {
        let context = taskContext.isEmpty ? "未绑定".L() : taskContext
        return """
        自动交互循环门禁：\(phase.title)
        任务上下文：\(context)
        已发送轮次：\(sentInputs.count)/\(maxTurns)
